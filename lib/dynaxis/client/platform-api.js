 ...
 */
export function createPlatformFetchError(res, data = {}) {
  const err = new Error(
    typeof data.error === 'string' && data.error ? data.error : `Platform API ${res.status}`
  );
...
/**
 * Session-authenticated fetch for Better Auth Workspace routes (no legacy owner_ref).
 * @param {string} path
 * @param {RequestInit & { query?: Record<string, string|undefined> }} [opts]
 */
export async function sessionPlatformFetch(path, opts = {}) {
  ...
  if (!res.ok) {
    throw createPlatformFetchError(res, data);
  }
  return data;
}

/**
 * Session-authenticated Project APIs (canonical workspace scope; no owner_ref).
 * @param {{ includeArchived?: boolean, ensureDefault?: boolean }} [query]
 */
export async function sessionListProjects(query = {}) {
  const q = {
    ...(query.includeArchived ? { includeArchived: 'true' } : {}),
    ...(query.ensureDefault === false ? { ensureDefault: 'false' } : { ensureDefault: 'true' }),
  };
  const data = await sessionPlatformFetch('/api/dynaxis/projects', { query: q });
 ...
 */
export async function sessionGetProject(projectId) {
  const data = await sessionPlatformFetch(`/api/dynaxis/projects/${projectId}`);
  return data.project ?? null;
}

/**
 * @param {{ name: string, description?: string | null, metadata?: Record<string, unknown> }} body
 */
export async function sessionCreateProject(body) {
  const data = await sessionPlatformFetch('/api/dynaxis/projects', {
    method: 'POST',
 ...
 */
export async function sessionUpdateProject(projectId, body) {
  const data = await sessionPlatformFetch(`/api/dynaxis/projects/${projectId}`, {
    method: 'PATCH',
    body: JSON.stringify(body),
  });
 ...
 * @param {string} projectId
 */
export async function sessionArchiveProject(projectId) {
  return sessionUpdateProject(projectId, { status: 'archived' });
}
