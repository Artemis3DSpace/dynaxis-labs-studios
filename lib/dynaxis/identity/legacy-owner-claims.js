...
import {
  createDrizzleWorkspaceOwnershipRepository,
...
export function createDrizzleLegacyOwnerClaimRepository(db = getDb()) {
  return {
    ...createDrizzleWorkspaceOwnershipRepository(db),
...
export async function claimLegacyOwner(
  ...
  return repository.transaction(async (tx) => {
    ...
    if (existing) {
      await projectWorkspaceOwnershipForLegacyOwnerRef(
        { legacyOwnerRef, organizationId: targetOrganizationId },
        { repository: tx }
      );
      return existing;
    }
