...
import {
  ...
  dynaxisWorkspaceAccessControl,
  dynaxisWorkspaceRoles,
} from './workspace-access.js';
import {
  DYNAXIS_AUTH_APP_NAME,
  DYNAXIS_AUTH_BASE_PATH,
  DYNAXIS_AUTH_MAX_PASSWORD_LENGTH,
  DYNAXIS_AUTH_MIN_PASSWORD_LENGTH,
} from './constants.js';
...
export function createDynaxisOrganizationPlugin({ organizationHooks } = {}) {
  return organization({
...
export function createDynaxisAuthOptions({
  database,
  env = process.env,
  databaseHooks,
  organizationHooks,
  ...
  const options = {
    ...
    databaseHooks,
    plugins: [createDynaxisOrganizationPlugin({ organizationHooks })],
  };
...
export function summarizeDynaxisAuthOptions(options) {
  return {
    ...
    databaseHooks: {
      sessionCreateBefore: Boolean(options.databaseHooks?.session?.create?.before),
    },
    organization: {
      importPath: 'better-auth/plugins',
      allowUserToCreateOrganization: options.plugins?.[0]?.options?.allowUserToCreateOrganization,
      disableOrganizationDeletion: options.plugins?.[0]?.options?.disableOrganizationDeletion,
      teamsEnabled: Boolean(options.plugins?.[0]?.options?.teams?.enabled),
      dynamicAccessControlEnabled: Boolean(
        options.plugins?.[0]?.options?.dynamicAccessControl?.enabled
      ),
      access: DYNAXIS_WORKSPACE_ACCESS_SUMMARY,
    },
