...
export function normalizeProjectAccess(input = {}) {
  ...
  const role = String(membership?.role || project.role || '').trim();
  const workspaceId = normalizeWorkspaceId(input.workspace);
  const projectWorkspaceId = normalizeProjectWorkspaceId(project) || String(membership?.organizationId || '').trim();
  const projectId = normalizeProjectId(project) || String(membership?.projectId || '').trim();
  ...
  return {
    ...
    isMember: Boolean(membership || project.isMember === true),
