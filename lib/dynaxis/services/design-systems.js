...
import { dynaxisDesignSystems } from '../db/schema.js';
...
export async function findDesignSystemOwnership(designSystemId) {
  const id = String(designSystemId || '').trim();
  if (!id) {
    return null;
  }
  const store = await getPlatformStore();
  if (typeof store.findDesignSystemOwnership === 'function') {
    return store.findDesignSystemOwnership(id);
  }
  ...
  const rows = await db
    .select({
      id: dynaxisDesignSystems.id,
      organizationId: dynaxisDesignSystems.organizationId,
    })
    .from(dynaxisDesignSystems)
    .where(eq(dynaxisDesignSystems.id, id))
    .limit(1);
  const row = rows[0];
  if (!row) {
    return null;
...
export const designSystemOwnershipRepository = Object.freeze({
  async findResource({ type, id }) {
    if (type !== 'design_system') {
      return null;
    }
    return findDesignSystemOwnership(id);
  },
});
