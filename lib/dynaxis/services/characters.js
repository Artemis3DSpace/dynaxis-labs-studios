...
import { dynaxisCharacters } from '../db/schema.js';
...
const memoryCharacterOwnershipRegistry = new Map();

export function clearCharacterOwnershipRegistry() {
  memoryCharacterOwnershipRegistry.clear();
}

function rememberCharacterOwnership(character) {
  if (!character?.id || !isMemoryDriverAllowed()) return;
  memoryCharacterOwnershipRegistry.set(character.id, {
    ownerRef: character.ownerRef,
    organizationId: character.organizationId ?? null,
  });
}

export function normalizeServiceContext(input) {
...
export async function resolveRouteServiceContext(routeContext) {
  const authContext = routeContext?.authContext || null;
  const workspace = authContext?.workspace || null;
  const organizationId = workspace?.organizationId || null;
  let ownerRef = null;

  if (authContext?.subject?.type === AUTH_CONTEXT_SUBJECT_TYPES.LEGACY) {
    ownerRef = authContext.subject.legacyOwnerRef || null;
  } else if (
    routeContext?.legacyCompatibility?.used === true &&
    routeContext.legacyCompatibility.ownerRef
  ) {
    ownerRef = routeContext.legacyCompatibility.ownerRef;
  } else if (organizationId) {
    const claims = await listLegacyOwnerRefClaimsForOrganization(organizationId);
    if (claims.length === 1) {
      ownerRef = claims[0].legacyOwnerRef;
    }
  }
  ...
  return Object.freeze({
    ...
    projectId: authContext?.project?.projectId || null,
  });
}
...
async function organizationIdForWrite(ctx, ownerRef) {
  const context = normalizeServiceContext(ctx);
  return (
    context.organizationId || (await resolveWorkspaceOrganizationForLegacyWrite(ownerRef))
  );
}

async function buildCharacterOwnershipMetadata(row) {
...
export async function findCharacterOwnership(characterId) {
  const store = await getPlatformStore();
  if (typeof store.findCharacterOwnership === 'function') {
    return store.findCharacterOwnership(characterId);
  }

  if (isMemoryDriverAllowed()) {
    const entry = memoryCharacterOwnershipRegistry.get(characterId);
    if (!entry) return null;
    return buildCharacterOwnershipMetadata({ id: characterId, ...entry });
  }

  const db = getDb();
  const [row] = await db
    .select({
      id: dynaxisCharacters.id,
      ownerRef: dynaxisCharacters.ownerRef,
      organizationId: dynaxisCharacters.organizationId,
    })
    .from(dynaxisCharacters)
    .where(eq(dynaxisCharacters.id, characterId))
    .limit(1);
  if (!row) return null;
  return buildCharacterOwnershipMetadata(row);
}

export const characterOwnershipRepository = Object.freeze({
  async findResource(input) {
    const type = String(input?.type || '').trim();
    if (type !== 'character' && type !== 'project_character') {
      return null;
    }
    const id = String(input?.id || '').trim();
    if (!id) return null;
    const characterId = type === 'project_character' && id.includes(':') ? id.split(':').pop() : id;
    return findCharacterOwnership(characterId);
  },
});
 ...
 */
export async function createRevisionForCharacter(ctx, characterId, { reason } = {}) {
...
export async function createCharacter(ctx, input) {
  ...
  rememberCharacterOwnership(character);

  const revision = await createRevisionForCharacter(ctx, character.id, {
    reason: 'create',
  });
...
export async function getCharacter(ctx, id, { includeAssets = false } = {}) {
  const ownerRef = await ownerRefFromContext(ctx);
  const store = await getPlatformStore();
...
export async function listCharacters(ctx, opts = {}) {
  const ownerRef = await ownerRefFromContext(ctx);
  const store = await getPlatformStore();
  return store.listCharacters(ownerRef, opts);
}

export async function updateCharacter(ctx, id, input) {
  ...
  rememberCharacterOwnership(character);
  const revision = await createRevisionForCharacter(ctx, id, { reason: 'profile_update' });
  return {
...
export async function archiveCharacter(ctx, id) {
  return updateCharacter(ctx, id, { status: 'archived' });
}

export async function addCharacterAsset(ctx, characterId, input) {
  ...
  if (data.createRevision) {
    revision = await createRevisionForCharacter(ctx, characterId, {
      reason: 'asset_link',
    });
  }
...
export async function removeCharacterAsset(ctx, characterId, assetId) {
  const ownerRef = await ownerRefFromContext(ctx);
  const store = await getPlatformStore();
  const character = await requireCharacter(store, ownerRef, characterId);
  ...
  const revision = await createRevisionForCharacter(ctx, characterId, {
    reason: 'asset_unlink',
  });
  return { removed: true, revision };
}

export async function listCharacterAssets(ctx, characterId) {
  const ownerRef = await ownerRefFromContext(ctx);
  const store = await getPlatformStore();
  await requireCharacter(store, ownerRef, characterId);
 ...
 */
export async function promoteAssetToReference(ctx, characterId, assetId, opts = {}) {
  return addCharacterAsset(ctx, characterId, {
    assetId,
...
export async function linkCharacterToProject(ctx, characterId, projectId) {
  const ownerRef = await ownerRefFromContext(ctx);
  const store = await getPlatformStore();
  await requireCharacter(store, ownerRef, characterId);
...
export async function unlinkCharacterFromProject(ctx, characterId, projectId) {
  const ownerRef = await ownerRefFromContext(ctx);
  const store = await getPlatformStore();
  await requireCharacter(store, ownerRef, characterId);
  const project = await resolveProjectId(ownerRef, projectId);
...
export async function listCharacterProjects(ctx, characterId) {
  const ownerRef = await ownerRefFromContext(ctx);
  const store = await getPlatformStore();
  await requireCharacter(store, ownerRef, characterId);
...
export async function listCharacterRevisions(ctx, characterId) {
  const ownerRef = await ownerRefFromContext(ctx);
  const store = await getPlatformStore();
  await requireCharacter(store, ownerRef, characterId);
...
export async function getCharacterRevision(ctx, revisionId) {
  const ownerRef = await ownerRefFromContext(ctx);
  const store = await getPlatformStore();
