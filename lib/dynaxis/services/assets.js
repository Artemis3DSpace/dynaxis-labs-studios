...
import { resolveCanonicalProjectId, resolveProjectId } from './projects.js';

export function createAssetOwnershipRepository(storeLoader = getPlatformStore) {
  return {
...
export const assetOwnershipRepository = createAssetOwnershipRepository();
 ...
 * @param {z.infer<typeof registerAssetSchema>} input
 */
export async function registerCanonicalAsset({ organizationId, userId, projectId }, input) {
  const data = registerAssetSchema.parse(input);
  const project = await resolveCanonicalProjectId({
    organizationId,
    userId,
...
export async function getCanonicalAsset(assetId) {
  const store = await getPlatformStore();
  return store.getCanonicalAsset(assetId);
}

export async function listCanonicalAssetsForProject({ projectId, generationId, limit = 50 }) {
  const store = await getPlatformStore();
  return store.listCanonicalAssetsForProject({ projectId, generationId, limit });
}
...
export const assetServiceImpl = {
  ...
  getCanonical: getCanonicalAsset,
  listCanonicalForProject: listCanonicalAssetsForProject,
  ownershipRepository: assetOwnershipRepository,
};
