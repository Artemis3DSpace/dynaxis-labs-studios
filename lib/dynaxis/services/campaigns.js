 ...
 */
export async function createRevisionForCampaign(ownerRef, campaignId, { reason } = {}) {
...
export async function createCampaign(ownerRef, input) {
  ...
  const revision = await createRevisionForCampaign(ownerRef, campaign.id, {
    reason: 'create',
  });
  const result = await getCampaign(ownerRef, campaign.id, { includeRelations: true });
  return { campaign: result.campaign, revision, ...result };
}

export async function updateCampaign(ownerRef, campaignId, input) {
  ...
  await createRevisionForCampaign(ownerRef, campaignId, { reason: 'update' });
  return getCampaign(ownerRef, campaignId, { includeRelations: true });
}

export async function archiveCampaign(ownerRef, campaignId) {
  return updateCampaign(ownerRef, campaignId, { status: 'archived' });
}
...
export async function listCampaigns(ownerRef, opts = {}) {
  const store = await getPlatformStore();
  const campaigns = await store.listCampaigns(ownerRef, opts);
  return { campaigns };
}

export async function linkCampaignProduct(ownerRef, campaignId, input) {
  ...
  await createRevisionForCampaign(ownerRef, campaignId, { reason: 'product_link' });
  return { link };
}

export async function unlinkCampaignProduct(ownerRef, campaignId, productId) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
  ...
  await createRevisionForCampaign(ownerRef, campaignId, { reason: 'product_unlink' });
  return { ok: true };
}

export async function listCampaignProducts(ownerRef, campaignId) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
  const products = await store.listCampaignProducts(campaignId);
  return { products };
...
export async function linkCampaignCharacter(ownerRef, campaignId, input) {
  ...
  await createRevisionForCampaign(ownerRef, campaignId, {
    reason: 'character_link',
  });
  return { link };
}

export async function unlinkCampaignCharacter(ownerRef, campaignId, characterId) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
...
export async function listCampaignCharacters(ownerRef, campaignId) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
  const characters = await store.listCampaignCharacters(campaignId);
  return { characters };
...
export async function listCampaignRevisions(ownerRef, campaignId) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
...
export async function getCampaignRevision(ownerRef, revisionId) {
  const store = await getPlatformStore();
...
export async function generateCampaignConcepts(
  ...
  if (!campaignRevisionId) {
    const revision = await createRevisionForCampaign(ownerRef, campaignId, {
...
export async function listCampaignConcepts(ownerRef, campaignId) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
  const concepts = await store.listCampaignConcepts(ownerRef, campaignId);
  return { concepts };
...
export async function getCampaignConcept(ownerRef, conceptId) {
  const store = await getPlatformStore();
  const concept = await requireConcept(store, ownerRef, conceptId);
  return { concept };
}

export async function updateCampaignConcept(ownerRef, conceptId, input) {
...
export async function selectCampaignConcept(ownerRef, campaignId, conceptId) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
  const concept = await requireConcept(store, ownerRef, conceptId);
...
export async function createCampaignDeliverables(ownerRef, campaignId, input) {
 ...
 */
export async function generateCampaignDeliverableImage(ownerRef, deliverableId) {
  const store = await getPlatformStore();
  const deliverable = await requireDeliverable(store, ownerRef, deliverableId);
  const campaign = await requireCampaign(store, ownerRef, deliverable.campaignId);
  const concept = await requireConcept(store, ownerRef, deliverable.conceptId);
...
export async function attachCampaignAsset(ownerRef, campaignId, input) {
...
export async function listCampaignAssets(ownerRef, campaignId) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
...
export async function removeCampaignAsset(ownerRef, campaignId, assetId) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
...
export async function listCampaignDeliverables(ownerRef, campaignId, opts = {}) {
  const store = await getPlatformStore();
  await requireCampaign(store, ownerRef, campaignId);
...
export async function getCampaignDeliverable(ownerRef, deliverableId) {
  const store = await getPlatformStore();
  const deliverable = await requireDeliverable(store, ownerRef, deliverableId);
  return { deliverable };
}
