...
async function assertDesignAgentCompositionScope(authScope, composition) {
  if (!authScope || !composition) {
    return;
  }
  if (authScope.projectId && composition.projectId !== authScope.projectId) {
    throw notFound('COMPOSITION_NOT_FOUND', 'Composition not found');
  }
  if (authScope.organizationId) {
    const organizationId = await resolveProjectOrganization(composition.projectId);
    if (organizationId && organizationId !== authScope.organizationId) {
      throw notFound('COMPOSITION_NOT_FOUND', 'Composition not found');
    }
  }
}

function designAgentAuthScopeFromRoute(routeContext) {
  return {
    organizationId: routeContext?.authContext?.workspace?.organizationId || null,
    projectId: routeContext?.authContext?.project?.projectId || null,
    principal: routeContext?.authContext?.principal || null,
  };
}

export { designAgentAuthScopeFromRoute };

/**
 * Assemble controlled context for Design Agent (READ).
 */
export async function readDesignAgentContext(ownerRef, compositionId, opts = {}) {
  const store = await getPlatformStore();
  const composition = await requireCompositionOwned(store, ownerRef, compositionId);
  await assertDesignAgentCompositionScope(opts.authScope, composition);
...
export async function listRelevantAssets(ownerRef, compositionId, { limit = 24, authScope = null } = {}) {
  const store = await getPlatformStore();
  const composition = await requireCompositionOwned(store, ownerRef, compositionId);
  await assertDesignAgentCompositionScope(authScope, composition);
 ...
 */
export async function listRelevantTokens(ownerRef, compositionId, { limit = 48, q = null, authScope = null } = {}) {
  const store = await getPlatformStore();
  const composition = await requireCompositionOwned(store, ownerRef, compositionId);
  await assertDesignAgentCompositionScope(authScope, composition);
...
export async function planDesignChanges(ownerRef, input, opts = {}) {
  ...
  const authScope = opts.authScope || null;
  const ctx = await readDesignAgentContext(ownerRef, data.compositionId, { authScope });
  if (
  ...
  const assets = await listRelevantAssets(ownerRef, data.compositionId, {
    limit: 16,
    authScope,
  });
  const components = await listRelevantComponents(ownerRef, { limit: 16 });
  const tokens = await listRelevantTokens(ownerRef, data.compositionId, {
    limit: 32,
    authScope,
  });
 ...
 */
export async function applyDesignAgentOperations(ownerRef, input, opts = {}) {
 ...
 */
export async function prepareImageGeneration(ownerRef, input, opts = {}) {
 ...
 */
export async function attachGeneratedAsset(ownerRef, input, opts = {}) {
