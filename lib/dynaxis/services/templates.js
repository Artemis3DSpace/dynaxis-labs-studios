...
import { dynaxisDesignTemplates } from '../db/schema.js';
...
export async function findDesignTemplateOwnership(templateId) {
  const id = String(templateId || '').trim();
  if (!id) {
    return null;
  }
  const store = await getPlatformStore();
  if (typeof store.findDesignTemplateOwnership === 'function') {
    return store.findDesignTemplateOwnership(id);
  }
  ...
  const rows = await db
    .select({
      id: dynaxisDesignTemplates.id,
      organizationId: dynaxisDesignTemplates.organizationId,
    })
    .from(dynaxisDesignTemplates)
    .where(eq(dynaxisDesignTemplates.id, id))
    .limit(1);
  const row = rows[0];
  if (!row) {
    return null;
...
export const designTemplateOwnershipRepository = Object.freeze({
  async findResource({ type, id }) {
    if (type !== 'design_template') {
      return null;
    }
    return findDesignTemplateOwnership(id);
  },
});
