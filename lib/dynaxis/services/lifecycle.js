 ...
 *   → MuAPI remains the execution engine
 */

import { z } from 'zod';
import { createGeneration, updateGenerationStatus, getGeneration } from './generations.js';
import { createJob, updateJob, markJobSucceeded, markJobFailed, getJob } from './jobs.js';
import { registerAsset } from './assets.js';
import { resolveProjectId } from './projects.js';
import { extractOutputUrls, safeProviderPayload } from '../providers/muapi.js';
import { inferAssetType, PROVIDER_MUAPI } from '../types.js';
 ...
 */
export async function startLifecycle(ownerRef, input) {
...
/**
 * Persist MuAPI request_id after submit.
 */
export async function attachProviderJobId(ownerRef, input) {
  const data = providerIdSchema.parse(input);
  const generation = await getGeneration(ownerRef, data.generationId);
  const job = await getJob(ownerRef, data.jobId);
  if (!generation || !job) {
