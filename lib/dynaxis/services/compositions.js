...
import { dynaxisCompositions, dynaxisProjects } from '../db/schema.js';
import { getApiKeyFromRequest } from '../ownership.js';
import {
  AuthContextError,
...
/** WP-7C-17 — AuthContext route boundary options for Design / Composition APIs. */
export const DESIGN_ROUTE_LEGACY_COMPAT = Object.freeze({ legacyCompatibility: true });

function partitionOwnerRef(routeContext) {
  const legacyAudit = routeContext?.legacyCompatibility;
  if (legacyAudit?.used && legacyAudit?.ownerRef) {
    return legacyAudit.ownerRef;
  }
  const subject = routeContext?.authContext?.subject;
  if (subject?.legacyOwnerRef) {
    return subject.legacyOwnerRef;
  }
  const compatibilityOwnerRef = routeContext?.authContext?.compatibility?.ownerRef;
  if (compatibilityOwnerRef) {
    return compatibilityOwnerRef;
  }
  const projectOwnerRef = routeContext?.authContext?.project?.ownerRef;
  if (projectOwnerRef) {
...
/**
 * Resolve the legacy owner_ref partition for Design-domain persistence calls.
 * Canonical session subjects without a projected partition stop at the route boundary.
 */
export function resolveRouteOwnerRef(routeContext) {
  const ownerRef = partitionOwnerRef(routeContext);
  if (!ownerRef) {
    throw new AuthContextError('Design route requires a legacy owner partition', {
...
/** Provider credential for LLM / generation adapters — never an identity subject. */
export function providerApiKeyFromRequest(request) {
  return getApiKeyFromRequest(request) || null;
}

export async function findCompositionOwnership(compositionId) {
  const id = String(compositionId || '').trim();
  if (!id) {
    return null;
  }
  const store = await getPlatformStore();
  if (typeof store.findCompositionOwnership === 'function') {
    return store.findCompositionOwnership(id);
  }
  ...
  const rows = await db
    .select({
      id: dynaxisCompositions.id,
      projectId: dynaxisCompositions.projectId,
      organizationId: dynaxisProjects.organizationId,
    })
    .from(dynaxisCompositions)
    .innerJoin(dynaxisProjects, eq(dynaxisCompositions.projectId, dynaxisProjects.id))
    .where(eq(dynaxisCompositions.id, id))
    .limit(1);
  const row = rows[0];
  if (!row) {
    return null;
...
export const compositionOwnershipRepository = Object.freeze({
  async findResource({ type, id }) {
    if (type !== 'composition') {
      return null;
    }
    return findCompositionOwnership(id);
  },
});

/**
 * Authorize a loaded Composition row through canonical project/resource inheritance.
 */
export async function requireCompositionRoutePermission(routeContext, composition, permission) {
  let resource = await findCompositionOwnership(composition?.id);
  if (!resource && composition?.id && composition?.projectId) {
    const organizationId = await resolveProjectOrganization(composition.projectId);
    resource = {
      type: 'composition',
      id: composition.id,
      projectId: composition.projectId,
  ...
  if (!resource) {
    throw new AuthContextError('Composition not found', {
      code: AUTH_CONTEXT_ERROR_CODES.PERMISSION_DENIED,
      status: 404,
    });
  }
  const decision = await checkAuthContextPermission(routeContext.authContext, permission, {
    projectId: resource.projectId,
    resource,
    resourceId: resource.id,
    resourceType: 'composition',
    resourceRepository: compositionOwnershipRepository,
  });
  if (!decision.allowed) {
    throw new AuthContextError('Composition route permission denied', {
      code: AUTH_CONTEXT_ERROR_CODES.PERMISSION_DENIED,
