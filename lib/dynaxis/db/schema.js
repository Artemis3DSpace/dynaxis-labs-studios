...
export function parseProjectRole(role) {
  const normalized = String(role || '').trim();
  return DYNAXIS_PROJECT_ROLE_NAMES.includes(normalized) ? normalized : null;
}
...
export const dynaxisProjects = pgTable(
  'dynaxis_projects',
  {
    ...
    // Nullable since WP-7C-24: canonical Workspace-owned Projects persist
    // organization_id ownership and carry no legacy owner_ref. Legacy Projects
    // continue to populate owner_ref for the compatibility partition.
  ...
  (t) => ({
    ...
    idOrganizationUidx: uniqueIndex('dynaxis_projects_id_organization_uidx').on(
      t.id,
      t.organizationId
    ),
  })
...
export const dynaxisProjectMembers = pgTable(
  'dynaxis_project_members',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    projectId: uuid('project_id')
      .notNull()
      .references(() => dynaxisProjects.id, { onDelete: 'cascade' }),
  ...
  (t) => ({
    projectUserUidx: uniqueIndex('dynaxis_project_members_project_user_uidx').on(
      t.projectId,
      t.userId
    ),
    projectIdx: index('dynaxis_project_members_project_idx').on(t.projectId),
    userIdx: index('dynaxis_project_members_user_idx').on(t.userId),
    organizationIdx: index('dynaxis_project_members_organization_idx').on(t.organizationId),
    projectOrganizationFk: foreignKey({
      columns: [t.projectId, t.organizationId],
      foreignColumns: [dynaxisProjects.id, dynaxisProjects.organizationId],
      name: 'dynaxis_project_members_project_organization_fk',
    }).onDelete('cascade'),
    workspaceMemberFk: foreignKey({
      columns: [t.organizationId, t.userId],
      foreignColumns: [member.organizationId, member.userId],
      name: 'dynaxis_project_members_workspace_member_fk',
    }).onDelete('cascade'),
    roleCheck: check(
      'dynaxis_project_members_role_check',
...
export const dynaxisAssets = pgTable(
  ...
  {
    id: uuid('id').defaultRandom().primaryKey(),
    // Nullable since WP-7C-24: canonical Assets inherit Workspace ownership
    // through their Project and carry no legacy owner_ref. Legacy Assets
    // continue to populate owner_ref for the compatibility partition.
    ownerRef: text('owner_ref'),
    projectId: uuid('project_id')
      .notNull()
...
export const dynaxisProjectsRelations = relations(dynaxisProjects, ({ many }) => ({
  members: many(dynaxisProjectMembers),
  generations: many(dynaxisGenerations),
  assets: many(dynaxisAssets),
  jobs: many(dynaxisJobs),
}));

export const dynaxisProjectMembersRelations = relations(dynaxisProjectMembers, ({ one }) => ({
  project: one(dynaxisProjects, {
    fields: [dynaxisProjectMembers.projectId],
    references: [dynaxisProjects.id],
  }),
  organization: one(organization, {
    fields: [dynaxisProjectMembers.organizationId],
    references: [organization.id],
  }),
  user: one(user, {
    fields: [dynaxisProjectMembers.userId],
    references: [user.id],
  }),
  workspaceMember: one(member, {
    fields: [dynaxisProjectMembers.organizationId, dynaxisProjectMembers.userId],
    references: [member.organizationId, member.userId],
  }),
}));
...
export const schema = {
  ...
  dynaxisProjectsRelations,
  dynaxisProjectMembersRelations,
  dynaxisGenerationsRelations,
};
