...
| Provider connection persistence and secrets | HARD | WP-7D-03, WP-7D-04 | ProviderConnection schema and encrypted-secret references are one persistence boundary. WP-7D-03 is integrated (migration `0015`) and owns `lib/dynaxis/provider-connections/schema.js`, `lib/dynaxis/secrets/schema.js`, and their registration in `lib/dynaxis/db/client.js`. | No Phase 7D migration owner is active. WP-7D-04 may now add runtime services under `lib/dynaxis/provider-connections/**` and `lib/dynaxis/secrets/**`, but must not alter the integrated `0015` schema without claiming a new explicit migration owner. |
