...
| Provider connection persistence and secrets | HARD | WP-7D-03 | ProviderConnection schema and encrypted-secret references are one persistence boundary. WP-7D-03 is the active Phase 7D migration owner (migration `0015`), owning `lib/dynaxis/provider-connections/schema.js`, `lib/dynaxis/secrets/schema.js`, and their registration in `lib/dynaxis/db/client.js`. | No other 7D package owns schema migration. WP-7D-04 may add services under `lib/dynaxis/provider-connections/**` and `lib/dynaxis/secrets/**` only after WP-7D-03 integrates. |
